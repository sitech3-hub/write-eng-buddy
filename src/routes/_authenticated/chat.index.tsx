...
function NewPracticePage() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col overflow-y-auto px-6 py-16">
      <div className="mb-12">
        <div className="mb-5 inline-flex items-center gap-1.5 rounded-full border bg-muted/40 px-2.5 py-1 text-[11px] font-medium text-muted-foreground">
          <Sparkles className="h-3 w-3" />
          새 연습
        </div>
        <h1 className="text-3xl font-semibold tracking-tight">무엇을 써볼까요?</h1>
        <p className="mt-2 text-sm text-muted-foreground">레벨과 연습 유형을 선택하면 바로 시작할 수 있어요.</p>
      </div>
      ...
      <section className="w-full space-y-10">
        <div>
          <div className="mb-4 flex items-end justify-between gap-3">
            <div>
              <h2 className="text-sm font-semibold tracking-tight">학습 레벨</h2>
              <p className="mt-1 text-xs text-muted-foreground">학년에 맞춘 CEFR 수준으로 난이도가 조정돼요.</p>
            </div>
            <Button
              type="button"
              variant="ghost"
          ...
          {recommended && (
            <div className="mb-3 rounded-lg border bg-muted/30 px-3 py-2 text-xs text-foreground">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {LEVELS.map((l) => (
              <button
                ...
                className={cn(
                  "rounded-lg border bg-card px-3 py-3 text-left text-sm transition",
                  level === l.value
                    ? "border-foreground/80 shadow-[0_0_0_1px_var(--color-foreground)]/0 ring-1 ring-foreground/80"
                    : "border-border hover:border-foreground/30"
                )}
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-medium">{l.label}</span>
                  <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">{l.cefr}</span>
                </div>
        ...
        <div>
          <h2 className="mb-4 text-sm font-semibold tracking-tight">연습 유형</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            {TYPES.map((t) => (
              <button
                ...
                className={cn(
                  "rounded-lg border bg-card px-4 py-3 text-left text-sm transition",
                  type === t.value
        ...
        <Button onClick={start} disabled={loading} className="w-full rounded-lg" size="lg">
